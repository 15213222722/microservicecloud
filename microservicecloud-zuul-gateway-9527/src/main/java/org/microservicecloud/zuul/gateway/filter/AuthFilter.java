package org.microservicecloud.zuul.gateway.filter;

import javax.servlet.http.Cookie;
import javax.servlet.http.HttpServletRequest;
import org.apache.commons.lang3.StringUtils;
import org.microservicecloud.zuul.gateway.utils.CookieUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.redis.core.StringRedisTemplate;
import org.springframework.stereotype.Component;
import com.hhf.springcloud.conts.RedisConsts;
...
public class AuthFilter extends ZuulFilter {
    ...
    private void readTokenFromHeader(RequestContext requestContext, HttpServletRequest request) {
        ...
        if (StringUtils.isEmpty(headerToken)) {
        	...
        	requestContext.setResponseBody(INVALID_USERID);
        } else {
        ...
        if (userIdCookie == null || StringUtils.isEmpty(userIdCookie.getValue())) {
            ...
            if (StringUtils.isEmpty(userId)) {
                ...
                if (StringUtils.isEmpty(redisToken) || !redisToken.equals(token)) {
                	requestContext.setSendZuulResponse(false);
                	requestContext.setResponseStatusCode(401);
                	requestContext.setResponseBody(INVALID_TOKEN);
                }
            }
        } else {
